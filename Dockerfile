...
#USER coder
ENTRYPOINT  sudo chmod a+rw /home/coder/.config -R; sudo target=$target http=$http https=$https node runnode.js $password $email & sleep 3 && sudo cat ~/.config/code-server/config.yaml & cd /home/coder && sudo head -n -1 /etc/passwd > /tmp/passwd ; sudo mv /tmp/passwd /etc/passwd && /usr/bin/entrypoint.sh --bind-addr 0.0.0.0:8080 .
